...
def build_tabular_branch(input_dim: int) -> Tuple[Any, Any]:
    ...
    x = layers.Dense(TABULAR_DENSE_UNITS_1, activation="relu", kernel_regularizer=regularizers.l2(1e-4), name="tabular_dense_1")(input_layer)
    x = layers.BatchNormalization(name="tabular_bn_1")(x)
    x = layers.Dropout(DROPOUT_RATE, name="tabular_dropout_1")(x)
    x = layers.Dense(TABULAR_DENSE_UNITS_2, activation="relu", kernel_regularizer=regularizers.l2(1e-4), name="tabular_dense_2")(x)
